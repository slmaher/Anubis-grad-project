...
export default function MarketplaceManagement() {
  ...
  const [formData, setFormData] = useState({ name: '', description: '', price: '', category: 'jewelry', imageUrl: '' });
  ...
  const pickProductImage = async () => {
    try {
      ...
      if (permission.status !== 'granted') {
        Alert.alert('Permission needed', 'Please allow gallery access to choose a product image.');
        return;
      }
      ...
      const uploadResult = await uploadImageToCloudinary(selected.uri, 'products');
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (res.success) {
        ...
        setEditingId(null);
        setFormData({ name: '', description: '', price: '', category: 'jewelry', imageUrl: '' });
        fetchProducts();
      } else {
      ...
      <View style={styles.cardImageWrap}>
        {item.imageUrl ? (
          ...
          <View style={styles.cardImagePlaceholder}>
            <MaterialCommunityIcons name="image-outline" size={24} color="#B39E8C" />
          </View>
        )}
      </View>
      <View style={styles.cardInfo}>
        <Text style={styles.name}>{item.name}</Text>
        <Text style={styles.sub}>{item.category} • {item.price} LE</Text>
          ...
          <View style={styles.modalContent}>
            ...
            <ScrollView>
              ...
              <View style={styles.imageActionsRow}>
                <TouchableOpacity
                  style={[styles.pickImageBtn, uploadingImage && { opacity: 0.7 }]}
...
const styles = StyleSheet.create({
  ...
  list: { gap: 12 },
  card: { backgroundColor: '#fff', borderRadius: 12, padding: 16, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', gap: 12, elevation: 2 },
  cardImageWrap: {
    width: 56,
    height: 56,
    borderRadius: 12,
  ...
  input: { borderWidth: 1, borderColor: '#ECE5DE', borderRadius: 8, padding: 12, marginBottom: 16, color: '#2C2010' },
  modalHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 },
  closeBtn: { width: 32, height: 32, borderRadius: 16, alignItems: 'center', justifyContent: 'center', backgroundColor: '#F5EFE7' },
  ...
  imageActionsRow: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', gap: 12, marginBottom: 12 },
