...
export default function DonationManagement() {
  ...
  const handleSave = async () => {
    ...
    const url = editingCampaign ? `${API_URL}/donations/campaigns/${editingCampaign._id}` : `${API_URL}/donations/campaigns`;

    try {
      const response = await fetch(url, {
        method,
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
        body: JSON.stringify({ ...formData, goalAmount: Number(formData.goalAmount) })
      });
      const res = await response.json();
      if (res.success) {
  ...
  const deleteCampaign = (id) => {
    Alert.alert('Deactivate Campaign', 'Are you sure?', [
      ...
      { text: 'Deactivate', style: 'destructive', onPress: async () => {
        const token = await getAuthToken();
        await fetch(`${API_URL}/donations/campaigns/${id}`, { method: 'DELETE', headers: { Authorization: `Bearer ${token}` } });
        fetchCampaigns();
      }}
    ]);
  };
  ...
  const openEdit = (campaign) => {
    ...
    setFormData({ title: campaign.title, description: campaign.description, goalAmount: campaign.goalAmount.toString() });
    setModalVisible(true);
  };
  ...
  const renderCampaign = ({ item }) => (
    <View style={styles.card}>
      <View style={styles.cardInfo}>
        ...
        <Text style={styles.sub}>{item.currentAmount} / {item.goalAmount} EGP raised</Text>
      </View>
      <View style={styles.actions}>
        ...
        <TouchableOpacity onPress={() => deleteCampaign(item._id)}><MaterialCommunityIcons name="delete-outline" size={22} color="#FF6B6B" /></TouchableOpacity>
      </View>
    </View>
  );
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <TouchableOpacity style={styles.addBtn} onPress={() => { setEditingCampaign(null); setFormData({ title: '', description: '', goalAmount: '' }); setModalVisible(true); }}>
          <MaterialCommunityIcons name="plus" size={24} color="#fff" />
          <Text style={styles.addBtnText}>New Campaign</Text>
        </TouchableOpacity>
      </View>

      <FlatList data={campaigns} renderItem={renderCampaign} keyExtractor={item => item._id} contentContainerStyle={styles.list} />

      <Modal visible={modalVisible} animationType="slide" transparent>
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            <Text style={styles.modalTitle}>{editingCampaign ? 'Edit Campaign' : 'New Campaign'}</Text>
            <ScrollView>
              <TextInput style={styles.input} placeholder="Title" value={formData.title} onChangeText={t => setFormData({...formData, title: t})} />
              <TextInput style={styles.input} placeholder="Goal Amount (EGP)" keyboardType="numeric" value={formData.goalAmount} onChangeText={t => setFormData({...formData, goalAmount: t})} />
