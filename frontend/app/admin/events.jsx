...
export default function EventManagement() {
  ...
  const [formData, setFormData] = useState({ title: '', description: '', museum: '', startDate: '', endDate: '', location: '' });
  ...
  const renderEvent = ({ item }) => (
    <View style={styles.card}>
      <View style={styles.cardInfo}>
        ...
        <Text style={styles.sub}>{new Date(item.startDate).toLocaleDateString()} at {item.museum?.name}</Text>
      </View>
      <View style={styles.actions}>
        ...
        <TouchableOpacity onPress={() => deleteEvent(item._id)}><MaterialCommunityIcons name="trash-can-outline" size={22} color="#FF6B6B" /></TouchableOpacity>
      </View>
    </View>
  );
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <TouchableOpacity style={styles.addBtn} onPress={() => { setEditingEvent(null); setFormData({ title: '', description: '', museum: '', startDate: '', endDate: '', location: '' }); setModalVisible(true); }}>
          <MaterialCommunityIcons name="plus" size={24} color="#fff" />
          <Text style={styles.addBtnText}>Add Event</Text>
        </TouchableOpacity>
      </View>

      <FlatList data={events} renderItem={renderEvent} keyExtractor={item => item._id} contentContainerStyle={styles.list} />
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            <ScrollView>
              ...
              <TextInput style={styles.input} placeholder="Start Date (YYYY-MM-DD)" value={formData.startDate} onChangeText={t => setFormData({...formData, startDate: t})} />
              <TextInput style={styles.input} placeholder="End Date (YYYY-MM-DD)" value={formData.endDate} onChangeText={t => setFormData({...formData, endDate: t})} />
