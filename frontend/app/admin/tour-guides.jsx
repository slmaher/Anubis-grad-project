...
export default function TourGuideManagement() {
  ...
  const guideUserIds = useMemo(() => new Set(guides.map((g) => g.user?._id).filter(Boolean)), [guides]);
    ...
    try {
      ...
      const body = {
        ...
        languages: formData.languages
          ...
          : undefined,
        experienceYears:
          formData.experienceYears !== '' ? Number(formData.experienceYears) : undefined,
      ...
      { text: 'Cancel', style: 'cancel' },
      {
        ...
        onPress: async () => {
          ...
          try {
            ...
            if (!response.ok || !res?.success) {
              Alert.alert('Delete failed', res?.message || 'Could not delete guide profile.');
              return;
            }
            await Promise.all([fetchGuides(), fetchUsers()]);
          } catch (error) {
  ...
  const renderGuide = ({ item }) => (
    <View style={styles.card}>
      <View style={styles.cardInfo}>
        ...
        <Text style={styles.sub}>{item.specialties?.join(', ') || 'No specialties'}</Text>
        <Text style={styles.rate}>{item.hourlyRate || 0} EGP/hr • {item.rating || 0} ★</Text>
      </View>
  ...
  if (loading) {
    return <ActivityIndicator size='large' color='#D9A441' style={{ flex: 1 }} />;
  }
  ...
  return (
    <View style={styles.container}>
      ...
      <FlatList
        ...
        ListEmptyComponent={<Text style={styles.emptyText}>No tour guide profiles yet.</Text>}
      />
          ...
          <View style={styles.modalContent}>
            ...
            <ScrollView>
              {!editingId && (
                <View style={styles.userPickerWrap}>
                  ...
                  {selectedUser ? (
                    ...
                    <Text style={styles.helper}>Choose a user to create a guide profile.</Text>
                  )}
                    ...
                    {users.map((user) => {
                      ...
                      return (
                        <TouchableOpacity
                          ...
                          disabled={disabled}
                          onPress={() => setFormData((prev) => ({ ...prev, user: user._id }))}
                        >
                          <Text style={styles.userChipName}>{user.name}</Text>
                          <Text style={styles.userChipEmail}>{user.email}</Text>
                          {hasProfile && <Text style={styles.userChipTag}>Already guide</Text>}
                        </TouchableOpacity>
                      );
                    })}
                  </ScrollView>
              ...
              <TextInput
                ...
                onChangeText={(t) => setFormData((prev) => ({ ...prev, specialties: t }))}
              />
              <TextInput
                style={styles.input}
                placeholder='Languages (comma separated)'
                value={formData.languages}
                onChangeText={(t) => setFormData((prev) => ({ ...prev, languages: t }))}
              />
              <TextInput
                style={styles.input}
                ...
                onChangeText={(t) => setFormData((prev) => ({ ...prev, experienceYears: t }))}
              />
              <TextInput
                style={styles.input}
                ...
                keyboardType='numeric'
                onChangeText={(t) => setFormData((prev) => ({ ...prev, hourlyRate: t }))}
              />
...
const styles = StyleSheet.create({
  ...
  userChipTag: { fontSize: 11, color: '#A5761A', marginTop: 6, fontWeight: '700' },
  input: {
    borderWidth: 1,
    borderColor: '#ECE5DE',
    borderRadius: 8,
