...
function isFriendId(list: Array<{ toString(): string }> | undefined, id: string) {
  ...
  async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
    try {
      ...
      const data = requests.map((request) => ({
        id: request.id,
        senderId: request.sender && typeof request.sender === 'object' ? request.sender._id : request.sender,
        senderName: request.sender && typeof request.sender === 'object' ? (request.sender as any).name : undefined,
        senderAvatar: request.sender && typeof request.sender === 'object' ? (request.sender as any).avatar : undefined,
...
friendsRouter.post(
  ...
  async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
    try {
      ...
      if (isFriendId(sender.friends, receiverId) || isFriendId(receiver.friends, senderId)) {
      ...
      if (existingRequest) {
        return res
          .status(409)
          .json({ success: false, message: 'A pending friend request already exists' });
      }
      ...
      const populated = await request.populate('sender receiver', 'name email avatar role');
...
friendsRouter.post(
  ...
  async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
    try {
      ...
      await Promise.all([
        UserModel.findByIdAndUpdate(senderId, { $addToSet: { friends: receiverId } }),
        UserModel.findByIdAndUpdate(receiverId, { $addToSet: { friends: senderId } }),
      ...
      const updatedRequest = await FriendRequestModel.findById(request.id).populate(
        'sender receiver',
