...
museumsRouter.get("/", async (_req, res: Response, next: NextFunction) => {
  try {
    const museums = await MuseumModel.find({ isActive: true }).sort({
      createdAt: -1,
    });
...
museumsRouter.get("/:id", async (req, res: Response, next: NextFunction) => {
  try {
