    ...
    const tryModel = (modelIndex: number) => {
      if (modelIndex >= GROQ_MODELS.length) {
        const error = new Error(
          "All Groq models failed or are quota-limited. Please try again later.",
        );
        ...
        (res) => {
          ...
          res.on("end", () => {
            ...
            if (statusCode === 404 || statusCode === 429) {
              console.log(
                `[Groq] Model ${model} returned ${statusCode}, trying next model...`,
              );
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (history.length === 0) {
        return res
          .status(400)
          .json({ success: false, message: "history is required" });
      }
      ...
      if (safeHistory.length === 0) {
        return res
          ...
          .json({
            success: false,
            message: "history contains no valid messages",
          });
      }
